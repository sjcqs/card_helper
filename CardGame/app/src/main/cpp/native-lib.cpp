...
JNIEXPORT jint JNICALL Java_dcc_up_pt_cardgame_CameraFragment_findCards(
        JNIEnv* env, jobject, jlong addrGray, jlong addrRgba, jlong addrCards, jlongArray cardsAddrs){
    Mat& mat_gray = *(Mat*)addrGray;
    Mat& mat_rgba = *(Mat*)addrRgba;
    Mat& mat_cards = *(Mat*)addrCards;

    jsize length = env->GetArrayLength(cardsAddrs);
    ...
    int count = 0;
    for (int i = 0; i < length && i < contours.size(); ++i) {
