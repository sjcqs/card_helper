...
JNIEXPORT jint JNICALL Java_dcc_up_pt_cardgame_CameraFragment_findCards(
        ...
        jlongArray cardsAddrs, jlongArray previewAddrs, jfloatArray cardsX){
    ...
    jlong* preview_addresses = env->GetLongArrayElements(previewAddrs, 0);

    GaussianBlur(mat_gray,mat_blur, Size(5, 5), 1000);
    ...
    if (contours.size() > 1) {
        ...
        for (int i = 0; i < cards_count && i < contours.size(); ++i) {
            ...
            if (area >= 0.90 * averageArea) {
                ...
                Mat &preview = *(Mat*) preview_addresses[i];
                ...
                if (set[0] && set[1] && set[2] && set[3]) {
                    ...
                    warpPerspective(mat_thresh, warp1, trans, Size(card.size[0], card.size[1]));

                    if (rect.boundingRect().width > rect.boundingRect().height) {
                        rotate(warp, card, ROTATE_90_CLOCKWISE);
                        rotate(warp1, preview, ROTATE_90_CLOCKWISE);
                    } else {
                        warp.copyTo(card);
                        warp1.copyTo(preview);
                    }
